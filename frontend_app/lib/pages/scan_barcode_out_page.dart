...
class ScanBarcodeOutPage extends StatefulWidget {
  const ScanBarcodeOutPage({super.key});

  @override
  State<ScanBarcodeOutPage> createState() => _ScanBarcodeOutPageState();
}

class _ScanBarcodeOutPageState extends State<ScanBarcodeOutPage>
  ...
  Future<void> _submitStockOut() async {
    ...
    setState(() => _busy = true);
    try {
      final res = await ApiService.stockOut(barcode: code, qty: _qty);
      final total = (res['total_quantity'] ?? '-').toString();
      _toast('scan.toast_out_success'.tr(args: [total])); // ✅

      await _backToStock();
    } catch (e) {
      _toast('scan.toast_out_fail'.tr(args: [e.toString()])); // ✅
    } finally {
        ...
        child: Column(
          children: [
            _StatusHeader(
              ...
              title: 'scan.scan_out_header'.tr(), // ✅ แก้ให้แปลภาษาได้
              subtitle: 'scan.out_desc'.tr(), // ✅
            ),
            ...
            _SolidButton(
              ...
              label: _busy ? 'scan.processing'.tr() : 'scan.confirm_out'.tr(), // ✅
              onTap: _busy ? () {} : _submitStockOut,
            ),
          ],
        ),
      ),
